...
result <- fromJSON(file = "timeseries3.json") # data from the COVID19
testing <- read.csv("covid19test2.csv", header = TRUE)
testing_permillion <- read.csv("covid19test_permillion.csv", header = TRUE)
```
...
for (element in seq(startint, endint - 2 * interval, 1)){
  ...
  # pvaluesig_date <- c(pvaluesig_date, dates_resample[testperiod])
}
...
plot(
  ...
  ylim = c(0, max(max(timeseries_resample),topval)),
...
points(
  ...
  testtsint_resample*(topval/max(testtsint_resample)),
  type = "o",
  col="red",
  pch=18,
  lwd=2,
...
axis(
  ...
  labels = round(seq(0,max(cumsum(testtsint_resample_pm)), max(cumsum(testtsint_resample_pm))/length(seq(0,topval, scale)))[1:length(seq(0,topval, scale))], 0), 
  las=1,
  cex.axis=0.5,
...
legend(
  "topleft", 
       legend=c("cases/testing",
                "Testing per million (right axis)"
                # "lockdown started"
                ),
       col=c("black", "red"), 
       lty=c(1, 1),
